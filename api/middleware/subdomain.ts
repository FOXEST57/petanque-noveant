...
export const detectSubdomain = async (
    ...
    try {
        ...
        if (headerSubdomain) {
            ...
            console.log(
                "🎯 Sous-domaine détecté via header X-Club-Subdomain:",
                headerSubdomain
            );
        } else if (clubParam) {
        ...
        if (subdomain) {
            ...
            try {
                ...
                if (rows.length > 0) {
                    ...
                    console.log(
                        "🎯 Club détecté via paramètre/sous-domaine:",
                        subdomain,
                        "ID:",
                        req.clubId
                    );
            ...
            console.log(
                "🚪 Aucun sous-domaine détecté - Redirection vers la page d'entrée"
            );
            return res.redirect("/club-finder");
        }
        ...
        console.error(
            "❌ Erreur dans le middleware de détection de sous-domaine:",
            error
        );
 ...
 */
export const enforceClubIsolation = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
