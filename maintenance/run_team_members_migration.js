...
async function runTeamMembersMigration() {
  let connection;
  ...
  try {
    ...
    const migrationSQL = fs.readFileSync('database/migration_add_club_id_to_team_members.sql', 'utf8');
    ...
    const [columns] = await connection.execute('DESCRIBE team_members');
